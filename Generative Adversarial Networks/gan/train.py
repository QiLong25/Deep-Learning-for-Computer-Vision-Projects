...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            noise_batch = sample_noise(batch_size, noise_size)
            noise_batch = noise_batch.reshape((batch_size, noise_size, 1, 1)).to(device)
            fake_images = G(noise_batch).reshape((batch_size, input_channels, img_size, img_size)).to(device)
